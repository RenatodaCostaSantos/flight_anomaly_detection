...
# Caminho para os dados de voo preparados (pós data_preparation, pré feature engineering)
DATA_DIR = Path(__file__).parent.parent.parent / "data" / "03_primary"
...
# Voo normal mais longo disponível — não tem aspd_meas em alguns casos, típico em produção
...
def test_predict_rejects_insufficient_data(client):
    """
    API deve retornar 422 quando receber menos de 2020 leituras.

    O modelo precisa de pelo menos 2020 amostras para computar a janela
    FFT mais larga (2000) e uma predição com sliding window (20).
    """
    payload = {
...
def test_predict_fault_flight_structure(client, fault_flight_df):
    ...
    Usa as primeiras 7000 linhas para ser rápido — suficiente para
