...
Os dois modelos capturam fenômenos físicos distintos com perfis de erro opostos. O \texttt{all\_features} reage às \textit{consequências} da falha --- queda de energia específica, divergência entre \textit{throttle} e velocidade, acúmulo de erros de controle --- sendo altamente conservador: cada alarme tem 98{,}3\,\% de chance de ser genuíno, mas o modelo silencia quando os efeitos ainda não se consolidaram, o que explica as 7 falhas perdidas. Em contraste, o \texttt{fft\_features} detecta a assinatura espectral da degradação \textit{antes} do momento oficial da falha, capturando o caos vibracional e a perda de pico de potência do motor. Esse comportamento de \textit{precursor} é evidenciado pelas centenas a milhares de janelas de alarme que antecedem a falha em alguns voos (e.g., 1.455 janelas no voo 2018-10-18~\#3), mas compromete a precisão por alarme (0{,}402) e inviabiliza seu uso isolado em operações com custo operacional elevado por falso positivo.
